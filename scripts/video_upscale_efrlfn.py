#!/usr/bin/env python3
"""Upscale videos with the EfRLFN model ("fast mode") and FFmpeg pipes.

EfRLFN (https://github.com/EvgeneyBogatyrev/EfRLFN, MIT License) is a much
smaller network than Real-ESRGAN (~487K params vs. millions) and runs on the
whole frame at once instead of 128x128 tiles, so it needs no overlap/stitch
logic. Benchmarked on a 640x480 anime frame: ~135ms/frame on MPS vs. ~970ms
for the Real-ESRGAN CoreML backend (~7x faster), at the cost of visibly
softer/less detailed output. Intended as an optional "fast mode", not a
replacement for the default backend.
"""
...
from efrlfn.model import EfRLFN

ROOT = Path(__file__).resolve().parents[1]
MODEL_DIR = ROOT / "tools" / "onnx-models"
WEIGHTS_URLS = {
    2: "https://drive.google.com/uc?export=download&id=1VeoW94hN1X-8kxGXQSyR53YzRqF1htKQ",
    4: "https://drive.google.com/uc?export=download&id=1vJgrsz62IAMeS9i2ChDhQGO6UO1ZUXhr",
}
...
def weights_path(scale: int) -> Path:
    return MODEL_DIR / f"efrlfn-x{scale}.pt"
...
def ensure_weights(scale: int) -> Path:
    ...
    print(f"Downloading EfRLFN x{scale} weights: {WEIGHTS_URLS[scale]}", file=sys.stderr, flush=True)
...
def upscale_frame(network: torch.nn.Module, device: str, raw: bytes, width: int, height: int) -> bytes:
    ...
    tensor = torch.from_numpy(image).permute(2, 0, 1).unsqueeze(0).float().to(device) / 255.0
    with torch.no_grad():
